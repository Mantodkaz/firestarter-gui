...
import { API_BASE } from '../shared/api/endpoints';
...
const Links: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return links.filter((l) =>
      (l.remote_path || '').toLowerCase().includes(q) ||
      (l.custom_title || '').toLowerCase().includes(q) ||
      (l.custom_description || '').toLowerCase().includes(q) ||
      (l.link_hash || '').toLowerCase().includes(q)
    );
  }, [links, search]);
  ...
  const fetchLinks = useCallback(async () => {
    ...
    setError('');
    try {
      const result = await invoke<PublicLinkEntry[]>('list_public_links', { userId });
  ...
  const handleHistoryFileClick = useCallback((remotePath: string) => {
    setNewFileName(remotePath);
  }, []);

  const handleCreate = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    const rp = newFileName.trim();
    if (!rp || !userId) return;
    ...
    try {
      await invoke('create_public_link', {
        userId,
        remotePath: rp,
      ...
      setError(msg || 'Failed to create link');
    } finally {
      setCreating(false);
    }
  }, [userId, newFileName, newTitle, newDescription, fetchLinks]);

  const handleDelete = useCallback(async (linkHash: string) => {
    if (!userId) return;
    setLoading(true);
    setError('');
    try {
      await invoke('delete_public_link', { userId, linkHash });
      await fetchLinks();
    } catch (e: any) {
            ...
            <div style={{ overflowX: 'auto', marginTop: 0 }}>
              <table style={{ width: '100%', minWidth: 700, background: '#181818', borderRadius: 8, borderCollapse: 'separate', borderSpacing: 0 }}>
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr>
                      <td colSpan={6} style={{ color: '#888', textAlign: 'center', padding: 24 }}>No public links found.</td>
                    </tr>
                  ) : (
                    paginated.map((link) => {
                      const publicUrl = `${API_BASE}/publicDownload?hash=${encodeURIComponent(link.link_hash)}&preview=true`;
                      const directUrl = `${API_BASE}/publicDownload?hash=${encodeURIComponent(link.link_hash)}`;
                        ...
                        <tr key={link.link_hash} style={{ borderBottom: '1px solid #23272f' }}>
                          ...
                          <td style={{ padding: 10, maxWidth: 160, wordBreak: 'break-all' }}>
                            {link.remote_path.length > 32 ? (
                              <span title={link.remote_path}>
                                {link.remote_path.slice(0, 12)}...{link.remote_path.slice(-12)}
                              </span>
                            ) : (
                              link.remote_path
                            )}
                          ...
                          {/* Hash */}
                          <td style={{ padding: 10, maxWidth: 120, wordBreak: 'break-all' }}>
                            <span title={link.link_hash}>
                              {link.link_hash.slice(0, 8)}...{link.link_hash.slice(-6)}
                            </span>
                          </td>
                          ...
                          <td style={{ padding: 10, maxWidth: 260, wordBreak: 'break-all' }}>
                            <a
                              ...
                              style={{ color: '#ff9800', textDecoration: 'underline', fontSize: 13 }}
                            >
                              {publicUrl}
                            </a>
                          </td>
                          ...
                          <td style={{ padding: 10, display: 'flex', gap: 6 }}>
                            ...
                            <button
                              className="button"
                              style={{ background: '#c62828', minWidth: 60, padding: '6px 12px' }}
                              onClick={() => handleDelete(link.link_hash)}
                            >
                              Delete
                            </button>
                          </td>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 16, gap: 8 }}>
                <button className="button" style={{ minWidth: 32, padding: '4px 10px' }} disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
                  Prev
                </button>
                <span style={{ color: '#ccc' }}>Page {page} of {totalPages}</span>
                <button className="button" style={{ minWidth: 32, padding: '4px 10px' }} disabled={page === totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>
                  Next
                </button>
              </div>
